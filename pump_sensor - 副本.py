...
class OCRProcessor:
    def __init__(self):
        ...
        self.ocr_regions = []
        self.last_ocr_results = {}  # 新增：存储上一次成功的OCR结果
        self.load_config()
            ...
            for region in self.ocr_regions:
                ...
                try:
                    ...
                    if cleaned_text:
                        ...
                        # 更新上一次成功的结果
                        self.last_ocr_results[region_name] = cleaned_text
                        print(f"OCR识别: {region_name} = {cleaned_text}")
                    else:
                        # 使用上一次成功的结果
                        results[region_name] = self.last_ocr_results.get(region_name, "")
                        print(f"OCR识别失败: {region_name}，使用上一次的值: {results[region_name]}")
                except Exception as e:
