...
显示执行日志和控制执行过程
"""
import subprocess  # 新增：用于启动外部程序
import sys  # 新增：获取Python解释器路径
import time
import datetime
import threading
import pyautogui
...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                             ...
                             QCheckBox, QProgressBar, QMessageBox)  # 新增QMessageBox
...
class ExecutionWindow(QWidget):
    ...
    def __init__(self, config_name):
        ...
        self.ocr_excel_path = None  # 存储OCR结果写入的Excel路径
        self.pump_process = None  # 新增：存储泵控制程序的进程对象
        self.setup_ui()
        self.load_config()
    ...
    def load_config(self):
        ...
        # 尝试加载第二个配置文件（_2后缀）
        self.config2 = None
        config2_name = f"{self.config_name}_2"
        self.config2 = load_config(config2_name)
        ...
        if self.config2:
            # 从第二个配置获取OCR数据源路径
            data_source2 = self.config2.get("data_source")
            if data_source2 and data_source2.get("type") == "excel":
                self.ocr_excel_path = data_source2["path"]
            
            info_text += f"\n\n检测到第二个配置文件: {config2_name}"
            info_text += f"\nOCR区域数量: {len(self.config2.get('regions', []))}"
            if self.ocr_excel_path:
                info_text += f"\nOCR结果写入: {self.ocr_excel_path}"
    ...
    def start_pump_sensor(self):
        """启动泵和传感器控制程序"""
        try:
            # 获取当前Python解释器路径
            python_exe = sys.executable
            # 启动pump_sensor.py程序
            self.pump_process = subprocess.Popen([python_exe, "pump_sensor.py"])
            self.log("泵和传感器控制程序已启动", "INFO")
            return True
        except Exception as e:
            self.log(f"启动泵和传感器控制程序失败: {str(e)}", "ERROR")
            return False
    
    def stop_pump_sensor(self):
        """停止泵和传感器控制程序"""
        if self.pump_process:
            try:
                self.pump_process.terminate()
                self.pump_process.wait(timeout=5)
                self.log("泵和传感器控制程序已停止", "INFO")
            except Exception as e:
                self.log(f"停止泵和传感器控制程序失败: {str(e)}", "ERROR")
            finally:
    ...
    def start_operation(self):
        ...
        if self.running:
            return
        # 新增：处理起始时间点记录
        if not self.handle_start_time():
            return        
        # 启动泵和传感器控制程序
        if not self.start_pump_sensor():
            return
    ...
    def handle_start_time(self):
        ...
        if os.path.exists(filename):
            ...
            reply = QMessageBox.question(
                self,
                "时间点文件已存在",
                f"检测到 {filename} 文件已存在。\n是否更新为当前时间点？\n当前时间: {current_time}",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.No
            )
            
            if reply == QMessageBox.No:
                self.log(f"保留原有时间点文件 {filename}", "INFO")
                return True  # 继续执行
            # 如果选择Yes，则继续写入新时间点
        else:
            self.log(f"创建时间点文件 {filename}", "INFO")
        
        try:
            with open(filename, 'w') as f:
                f.write(current_time)
            self.log(f"已记录起始时间点: {current_time}", "INFO")
            return True
        except Exception as e:
            QMessageBox.critical(
                self,
                "写入时间点失败",
                f"无法写入时间点文件 {filename}:\n{str(e)}"
            )
            return False
    ...
    def run_normal_operations(self, window, operations, wx, wy):
        ...
        try:
            ...
            if not self.stop_flag:
                ...
                # 执行OCR识别（如果有第二个配置）
                if self.config2 and self.ocr_excel_path:
                    self.log_signal.emit("开始OCR识别...", "INFO")
                    self.perform_ocr_and_write_excel(window)
        except Exception as e:
    ...
    def perform_ocr_and_write_excel(self, window):
        """执行OCR识别并将结果写入Excel"""
        try:
            # 获取OCR区域
            ocr_regions = self.config2.get("regions", [])
            if not ocr_regions:
                self.log_signal.emit("第二个配置中没有OCR区域", "WARNING")
                return
            ...
            for region in ocr_regions:
                region_name = region.get("name", "").replace("Excel标签: ", "")
                if not region_name:
                    continue
                ...
                try:
                    ...
                    if cleaned_text:
                        ocr_results[region_name] = cleaned_text
                        self.log_signal.emit(f"OCR识别: {region_name} = {cleaned_text}", "INFO")
                    else:
                        self.log_signal.emit(f"OCR识别失败: {region_name}", "WARNING")
                except Exception as e:
                    self.log_signal.emit(f"OCR识别出错: {str(e)}", "ERROR")
            
            # 如果识别到结果，写入Excel
            #if ocr_results and self.ocr_excel_path:
            #    self.log_signal.emit(f"开始写入Excel: {self.ocr_excel_path}", "INFO")
            #    self.write_ocr_to_excel(ocr_results)
        except Exception as e:
            self.log_signal.emit(f"执行OCR识别时出错: {str(e)}", "ERROR")

    def write_ocr_to_excel(self, ocr_results):
        """将OCR结果写入Excel文件"""
        try:
            # 检查文件是否存在
            if not os.path.exists(self.ocr_excel_path):
                # 创建新Excel文件
                self.log_signal.emit(f"创建新Excel文件: {self.ocr_excel_path}", "INFO")
                wb = Workbook()
                ws = wb.active
                ws.title = "OCR Results"
                
                # 创建标题行
                headers = ["Timestamp"] + list(ocr_results.keys())
                ws.append(headers)
                next_timestamp = 1
            else:
                # 加载现有工作簿
                wb = load_workbook(self.ocr_excel_path)
                ws = wb.active
                ...
                # 获取最后一行的Timestamp值
                last_row = ws.max_row
                last_timestamp = ws.cell(row=last_row, column=1).value
                
                # 计算新的Timestamp（加1）
                try:
                    next_timestamp = int(last_timestamp) + 1
                except:
                    next_timestamp = 1
                
                # 检查是否有新标签需要添加
                new_headers = []
                for tag in ocr_results.keys():
                    if tag not in headers:
                        new_headers.append(tag)
            ...
            # 创建新行数据
            new_row = [next_timestamp]
            
            # 按标题顺序填充数据
            for header in headers[1:]:  # 跳过Timestamp列
                value = ocr_results.get(header, "")  # 获取对应标签的值
            ...
            # 保存工作簿
            wb.save(self.ocr_excel_path)
            self.log_signal.emit(f"已写入新行: Timestamp={next_timestamp}", "INFO")
        except Exception as e:
            self.log_signal.emit(f"写入Excel时出错: {str(e)}", "ERROR")
    ...
    def execute_operation(self, window, operation, step_index, wx=None, wy=None, wwidth=None, wheight=None):
        ...
        wait_time = operation.get("wait_time", 0.1)
        times = operation.get("config", {}).get("times", 0.1)
        ...
        if template_path and os.path.exists(template_path):
            try:
                ...
                screenshot_gray = cv2.cvtColor(screenshot_np, cv2.TM_CCOEFF_NORMED)
                ...
                if max_val >= threshold:
                    ...
                    if op_type == "click":
                        for t in range(times):
                            ...
                            pyautogui.press('enter')#输入回车键pyautogui.write(['enter'])
        ...
        if not found:
            ...
            if op_type == "click":
                ...
                pyautogui.write(text)
                #pyautogui.press('enter')#输入回车键pyautogui.write(['enter'])
                pyautogui.write(['enter'])#输入回车键pyautogui.write(['enter'])
                time.sleep(wait_time)
            elif op_type == "wait":
