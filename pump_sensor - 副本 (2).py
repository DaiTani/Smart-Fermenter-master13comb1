...
class DataCollector:
    ...
    def initialize_kalman_filters(self):
        ...
        filter_params = {
            # OD (COM3)
            0: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
            # electrical conductivity (COM3)
            1: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
            # fluorescence raw (COM6)
            2: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
            # fluorescence voltage (COM6)
            3: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
            # Hum%RH (COM7 dirt value)
            4: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
            # AD Value1 (COM7 ad value)
            5: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
            # Dirt2 (COM15 dirt value)
            6: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
            # AD Value2 (COM15 ad value)
            7: {"process_variance": 1e-5, "measurement_variance": 0.1, "initial_value": 0.0},
        }
        ...
        try:
            with open('sensor_data.csv', 'a', newline='') as f_merged:
                ...
                while not stop_event.is_set():
                                ...
                                if new_channel != current_channel:
                                    ...
                                    if not os.path.exists(file_path):
                                        with open(file_path, 'w', newline='') as f_new:
                                            csv.writer(f_new).writerow(csv_head[:-1])  # 修改：去掉最后一列OCR
                    ...
                    # 合并所有数据（时间戳 + 传感器数据 + OCR数据 + 日志）
                    if len(merged_data) == 8:  # 更新为8个数据点
                        timestamp = datetime.now().isoformat(timespec='milliseconds')
                        all_data = [timestamp] + merged_data + ocr_values
