...
class DataCollector:
    ...
    def serial_to_csv(self):
        ...
        # 备份逻辑保持不变
        backup_files = [
            os.path.join(base_dir, f'data{channel}.csv') for channel in range(1,5)
        ] + [
            os.path.join(base_dir, 'data.csv')
        ] + [
            os.path.join(base_dir, f'data6_{c}.csv') for c in range(1,5)
        ]
        ...
        # 修改主数据文件名
        main_file_path = os.path.join(base_dir, 'data.csv') 
        print(f"\n切换到通道 {main_file_path}，数据将保存至 {base_dir}")		
        # 修改CSV表头，添加OCR数据列
        ...
        with open(main_file_path, 'w', newline='') as f_merged:
        ...
        try:
            with open(main_file_path, 'a', newline='') as f_merged:
                        ...
                        if isinstance(cmd, tuple):
                            if cmd[0] == 'channel':
                                ...
                                if new_channel != current_channel:
                                    ...
                                    # 创建新通道文件（仅限普通通道）
                                    file_path = f'data{new_channel}.csv'
                                ...
                                file_path = f'data6_{parent_channel}.csv'
                                # 通道6文件单独管理
