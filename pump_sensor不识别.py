...
class DataCollector:
    ...
    def add_to_buffer(self, filtered_data):
        ...
        for i, value in enumerate(filtered_data):
            if len(self.sensor_buffer[i]) < self.buffer_size:
                ...
                #print(f"1111数据读取11: 11")
            else:
                # 缓冲区已满，替换最旧的数据
                self.sensor_buffer[i].pop(0)
                self.sensor_buffer[i].append(value)
                #print(f"0001111数据读取11: 11")
    ...
    def serial_to_excel(self):
        ...
        headers = ['Timestamp', 'electrical conductivity', 'Refractive', 'fluorescence', 
                   'fluorescence AD Value', 'Dirt1', 'AD Value1', 'Dirt2', 'od_600']
        print(f"\n切换到headers {headers}")
        # 添加OCR列标题
            ...
            while not stop_event.is_set():
                ...
                merged_data = [
                    *self.latest_com3_data,#electrical conductivity
                    ...
                    self.latest_com15_data[1] + self.latest_com7_data[1]# AD Value2
                ]
                ...
                if buffer_full:
                    ...
                    all_data = [timestamp] + processed_data + ocr_values
