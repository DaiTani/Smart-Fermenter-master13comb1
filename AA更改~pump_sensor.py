...
class DataCollector:
    def __init__(self):
        ...
        self.sensor_buffer = [[] for _ in range(9)]  ########## 8个传感器通道#############################################################需要手动修改
        self.buffer_size = 5  # 缓冲10个数据点
        self.buffer_count = 0        
        self.base_dir = r".\Data5\28"
        self.time_start = self.read_time_start()
        self.last_tstamp = self.read_last_timestamp()        

    def read_time_start(self):
        time_start_path = os.path.join('time_start.txt')
        try:
            with open(time_start_path, 'r') as f:
                start_str = f.read().strip()
            return datetime.fromisoformat(start_str)
        except Exception as e:
            print(f"无法读取 time_start.txt: {e}，使用当前时间作为基准")
            return datetime.now()

    def read_last_timestamp(self):
        data_path = os.path.join(self.base_dir, 'data.xlsx')
        ...
        try:
            ...
            last_timestamp_str = ws.cell(row=max_row, column=max_column).value
            #last_timestamp_str = ws.cell(row=max_row, column=1).value
            if not last_timestamp_str:
                return 0.0
            last_datetime = datetime.fromisoformat(last_timestamp_str)
            timestamp = (last_datetime - self.time_start).total_seconds()
            return round(timestamp / 20) * 20
        except Exception as e:
            print(f"读取 data.xlsx 最后 timestamp 失败: {e}")
            return 0.0    
    ...
    def initialize_kalman_filters(self):
        """为每个传感器数据创建卡尔曼滤波器"""
        # 定义每个传感器的初始参数
        # 过程噪声方差（系统不确定性）和测量噪声方差（传感器不确定性）########################################################################数量需要手动修改
        filter_params = {
            # OD (COM3)
            ...
            # AD Value2 (COM15 ad value)
            8: {"process_variance": 1e-6, "measurement_variance": 0.5, "initial_value": 0.0},
        }
    ...
    def safe_save_workbook(self, wb, file_path, max_attempts=10, wait_time=1):
        """
        带重试机制的安全保存工作簿函数
        :param wb: 工作簿对象
        ...
        while attempts < max_attempts:
            try:
                ...
                attempts += 1
                print(f"\n文件 {file_path} 被占用，保存失败 (尝试 {attempts}/{max_attempts})，等待 {wait_time} 秒后重试...")
                time.sleep(wait_time)
            except Exception as e:
                print(f"\n保存文件 {file_path} 时发生未知错误: {e}")
                return False
        return False        
    def serial_to_excel(self):
        ...
        # 从data_style.xlsx获取headers
        style_file_path = os.path.join(r".\Data5", 'data_style.xlsx')
        #headers = self.get_headers_from_style(style_file_path)
        #print(f"\n切换到headers {headers}")
        pr = cProfile.Profile()
        pr.enable()
        ...
        #        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        #        backup_name = os.path.join(base_dir, f"{filename.split('.')[0]}_{timestamp}.xlsx")
        #        os.rename(filename, backup_name)
        
        
        #headers = ['Timestamp', 'electrical conductivity', 'Refractive', 'fluorescence', 
        #           'fluorescence AD Value', 'Dirt1', 'AD Value1', 'Dirt2', 'induction', 'od_600']
        #print(f"\n切换到headers {headers}")
        # 添加OCR列标题
        ...
        if self.ocr_processor.ocr_regions:
            for region in self.ocr_processor.ocr_regions:
                region_name = region.get("name", "")
                if region_name:
                    ocr_headers.append(region_name)

        # 找到 'Timestamp' 的索引位置
        timestamp_index = headers.index('Timestamp')

        # 将OCR列插入到 'Timestamp' 之后
        headers = headers[:timestamp_index+1] + ocr_headers + headers[timestamp_index+1:]                    
        ...
        try:
            while not stop_event.is_set():
                ...
                # 构建传感器数据行############################################################################################################内容需要手动修改
                merged_data = [
                    *self.latest_com3_data,#electrical conductivity
                    self.latest_com6_data[0],#fluorescence
                    self.latest_com6_data[1],#fluorescence AD Value
                    self.latest_com7_data[0],# Dirt1
                    self.latest_com7_data[1],# AD Value1
                    self.latest_com15_data[0] + self.latest_com7_data[0],# Dirt2
                    0,# induction固定值
                    self.latest_com15_data[1] + self.latest_com7_data[1]#od_600
                ]
                #print(f'\r已保存: {merged_data}')
                # 应用卡尔曼滤波
                ...
                if buffer_full:
                    ...
                    #print(f'\r已保存: {current_time}')
                    delta_time = (current_time - self.time_start).total_seconds()
                    #print(f'\r已保存: {current_time}')
                    last_tstamp = self.last_tstamp
                    delta = delta_time - last_tstamp
                    print(f'\r已保存delta: {delta}')
                    #print(f'\r已保存last_tstamp: {last_tstamp}')
                    #print(f'\r已保存current_time: {delta_time}')
                    if delta >= 20:#######################################################################################################时间需要手动修改
                        new_tstamp = last_tstamp + 20
                    else:
                        wait_time = 20 - delta
                        start_wait = time.time()
                        remaining = wait_time
                        print(f"等待 {remaining:.2f} 秒以补足20秒间隔", end='', flush=True)
                        ...
                        while remaining > 0:
                            ...
                            print(f"\r等待 {remaining:.2f} 秒以补足20秒间隔", end='', flush=True)
                        new_tstamp = last_tstamp + 20

                    self.last_tstamp = new_tstamp
                    ...
                    all_data = [timestamp] + ocr_values + processed_data
                    
                    # 预处理所有数据，将0值替换为0.0001
                    preprocessed_data = self.preprocess_all_data(all_data)
                    self.last_log = current_time.strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    # 定期保存工作簿
                    #if time.time() - last_save > 20:  # 每30秒保存一次
                    #    merged_wb.save(main_file_path)
                    #    for key, (wb, path) in channel_wbs.items():
                    #        wb.save(path)
                    #    last_save = time.time()
                        
                    if time.time() - last_save > 20:  # 每20秒保存一次
                        # 保存主工作簿
                        if not self.safe_save_workbook(merged_wb, main_file_path):
                            print(f"主工作簿保存失败，稍后重试")
                        
                        # 保存各个通道工作簿
                        for key, (wb, path) in channel_wbs.items():
                            if not self.safe_save_workbook(wb, path):
                                print(f"通道工作簿 {path} 保存失败，稍后重试")
            ...
            # 最终保存所有工作簿
            if not self.safe_save_workbook(merged_wb, main_file_path, max_attempts=10, wait_time=1):
                print(f"警告：主工作簿最终保存失败！")
            
            for key, (wb, path) in channel_wbs.items():
                if not self.safe_save_workbook(wb, path, max_attempts=10, wait_time=1):
                    print(f"警告：通道工作簿 {path} 最终保存失败！")
    ...
    def get_headers_from_style(self, file_path):
        """从data_style.xlsx文件中提取dm_air标签之前的标签集合"""
        try:
            # 尝试加载样式文件
            work_dir="./Data5",
            wb = load_workbook(work_dir)
            ws = wb.active
            ...
            # 提取dm_air之前的标签
            if dm_air_index is not None:
                return list(header_row[:dm_air_index])
            else:
                # 如果没有找到dm_air，返回默认headers
                print("警告：在data_style.xlsx中未找到'dm_air'标签，使用默认headers")
                return ['Timestamp', 'OD', 'electrical conductivity', 'fluorescence', 
                        'Hum%RH', 'Dirt1', 'AD Value1', 'Dirt2', 'AD Value2']
        
        except Exception as e:
            print(f"无法加载data_style.xlsx文件: {e}")
            # 文件不存在或读取失败时使用默认headers
            return ['Timestamp', 'OD', 'electrical conductivity', 'fluorescence', 
                    'Hum%RH', 'Dirt1', 'AD Value1', 'Dirt2', 'AD Value2']
...
def plot_data():
    ...
    def update(frame):
        ...
        if timestamps:
            ...
            for ax in [ax1, ax2]:
                ax.set_xlim(0, max_time + 10)  # 留出10秒缓冲区
...
if __name__ == "__main__":
    ...
    def main():
        ...
        try:
            ...
            controller.pump1.safe_shutdown()
            controller.pump2.safe_shutdown()
            print("\n程序已安全退出")
