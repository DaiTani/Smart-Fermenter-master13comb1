...
def load_correction_data(data_path, pred_path, seq_length=20):
    ...
    # 动态获取标签
    all_columns = utils.get_data_style(data_dir)
    x_columns = [col for col in all_columns if col not in ["Timestamp", "od_600"]]
    print(f"true_valuex_columns: {x_columns}")
    # 使用动态特征列
    ...
    #print(f"true_value: {feature_columns}")	
    #features = merged_df[feature_columns].values
...
def main():
    ...
    all_columns = utils.get_data_style(args.dataset)
    x_columns = [col for col in all_columns if col not in ["Timestamp", "od_600"]]
    x_columnsALL = [col for col in all_columns if col not in ["####", "#####"]]
    print(f"seq_featuresx_columns: {x_columns}")
    n_features = len(x_columns)  
    print(f"seq_featuresn_features: {n_features}")
    # 初始化结果文件
    if not os.path.exists(result_file):
        pd.DataFrame(columns=x_columnsALL).to_excel(result_file, index=False)
    ...
    # 优化：确保文件操作后及时释放权限
    #############################################################
    print("等待数据文件满足条件（Timestamp列最后一个值 >= 350）...")
    while True:
        try:
            # 临时变量存储数据，确保快速释放文件
            df = None
